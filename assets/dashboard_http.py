...
class DashboardHttp:
    ...
    async def close_ticket(self, request: web.Request) -> web.Response:
        ...
        if len(reason) < 2:
            return web.json_response(
                {"error": "Reason must be at least 2 characters"}, status=400
            )

        guild_id = self._resolve_guild_id(body, channel_id)
        if guild_id is None:
            return web.json_response(
                {"error": "Could not resolve guild_id"}, status=400
            )
        ...
        try:
            ...
            return web.json_response(
                {"error": ErrorMessages.user_message_for(exc)}, status=500
            )
    ...
    async def execute_ticket_command(self, request: web.Request) -> web.Response:
        ...
        if actor is None:
            try:
                ...
                return web.json_response(
                    {"error": "Staff actor not found in guild"}, status=400
                )

        if channel.category is None or channel.category.id not in cfg.get(
            "TICKET_CATEGORIES", []
        ):
            return web.json_response(
                {"error": "This channel is not a ticket"}, status=400
            )
        ...
        try:
            if command == "close":
                if close_cog is None:
                    return web.json_response(
                        {"error": "Close cog not loaded"}, status=503
                    )
                if len(args) < 2:
                    return web.json_response(
                        {"error": "Usage: /close <reason>"}, status=400
                    )
            ...
            if command == "rename":
                if rename_cog is None:
                    return web.json_response(
                        {"error": "Rename cog not loaded"}, status=503
                    )
                new_name = args.strip()
                if len(new_name) < 2:
                    return web.json_response(
                ...
                return web.json_response(
                    {
                        "ok": True,
                        "command": command,
                        "detail": f"Renamed to {channel.name}",
                    }
                )
            ...
            if command == "add":
                if add_cog is None:
                    return web.json_response(
                        {"error": "Add cog not loaded"}, status=503
                    )
                ...
                if rows:
                    return web.json_response(
                        {"error": "User is ticket blacklisted"}, status=400
                    )
            ...
            if command == "remove":
                if remove_cog is None:
                    return web.json_response(
                        {"error": "Remove cog not loaded"}, status=503
                    )
                user_id = self._extract_snowflake(args)
                if not user_id:
                    return web.json_response(
            ...
            if command == "move":
                if move_cog is None:
                    return web.json_response(
                        {"error": "Move cog not loaded"}, status=503
                    )
                target = args.strip()
                if not target:
                    return web.json_response(
                ...
                if category is None:
                    return web.json_response(
                        {"error": "Target category not found"}, status=404
                    )
                ...
                if category.id not in cfg.get("TICKET_CATEGORIES", []):
                    return web.json_response(
                        {"error": "That is not a ticket category"}, status=400
                    )
                ...
                return web.json_response(
                    {
                        "ok": True,
                        "command": command,
                        "detail": f"Moved to {category.name}",
                    }
                )
            ...
            if command in ("private", "management"):
                if private_cog is None:
                    return web.json_response(
                        {"error": "Private cog not loaded"}, status=503
                    )
                if command == "private":
                    target_category = guild.get_channel(
                        cfg.get("CHANNEL_IDS", {}).get("ADMIN_PRIVATE_CATEGORY_ID")
                    )
                    privated = "Admin"
                ...
                if not isinstance(target_category, discord.CategoryChannel):
                    return web.json_response(
                        {"error": "Target category unavailable"}, status=503
                    )
                ...
                if staff_id:
                    ...
                    if staff_team:
                        await channel.set_permissions(staff_team, view_channel=False)
                await channel.send(
                    embed=await self._ticket_embed(guild_id, f"{actor.mention} {desc}")
                )
                return web.json_response(
            ...
            log.exception(
                "Dashboard ticket-command failed for %s/%s", channel_id, command
            )
            return web.json_response(
                {"error": ErrorMessages.user_message_for(exc)}, status=500
            )

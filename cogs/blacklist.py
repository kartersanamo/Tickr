...
class Blacklist(commands.Cog):
    ...
    async def blacklist_user(
        self, interaction: discord.Interaction, user: discord.Member, length: str, reason: str, guild_id: int
    ) -> None:
        ...
        DatabasePool.execute(
            ...
            (guild_id, user.id, reason or "N/A", interaction.user.id, unix, int(time.time())),
        )
        log_commands.info(f"Ticket blacklisted {user} ({user.id}) for {length}")

    @TaskDecorator.task("Send Embed", False)
    async def send_embed(
        self, interaction: discord.Interaction, user: discord.Member, blacklisted: str, cfg
    ) -> None:
        embed = discord.Embed(
    ...
    async def send_webhook(
        self, interaction: discord.Interaction, user: discord.Member, length: str, reason: str, cfg
    ) -> None:
    ...
    async def blacklist_command(
        self, interaction: discord.Interaction, user: discord.Member, length: str, reason: str = None
    ) -> None:
        ...
        if not blacklisted:
            await self.blacklist_user(interaction, user, length, reason, interaction.guild_id)

...
class Remove(commands.Cog):
    ...
    async def is_higher_rank(self, role_id1: int, role_id2: int, hierarchy: dict) -> bool:
    ...
    async def check_higher_rank(self, interaction: discord.Interaction, user: discord.Member, cfg) -> bool:
        ...
        if staff_team_role and staff_team_role in user.roles:
            ...
            role_id_1 = user.top_role.id if user.top_role.id not in disregard else user.roles[-2].id
            role_id_2 = (
            ...
            if await self.is_higher_rank(role_id_1, role_id_2, hierarchy):
                log_commands.warning(f"{interaction.user} tried to remove higher rank {user}")
